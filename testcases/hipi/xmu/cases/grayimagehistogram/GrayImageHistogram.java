package hipi.xmu.cases.grayimagehistogram;
...
public class GrayImageHistogram extends Configured implements Tool{
	...
	public static class MyMapper extends Mapper<ImageHeader, FloatImage, IntWritable, LongWritable>
	{
		...
		public void setup(Context jc) throws IOException
		{
			...
			path = new Path( conf.get("GrayImageHistogram.outdir"));
		...
		public void map(ImageHeader key, FloatImage value, Context context) throws IOException, InterruptedException{
			if (value != null) {
				...
				//smooth
				int[] rgb =graytool.HISTOGRAM(gray);
	...
	public static class MyReducer extends Reducer<IntWritable, LongWritable, IntWritable, LongWritable> {
		...
		{	
			...
			if (args.length < 3)
			{
				System.out.println("Usage: GrayImageHistogram <inputdir> <outputdir> <input type: hib, har, sequence, small_files>");
				System.exit(0);
			}
			...
			conf.setStrings("GrayImageHistogram.outdir", outputPath);
			conf.setStrings("GrayImageHistogram.filetype", input_file_type);

			Job job = new Job(conf, "GrayImageHistogram");
			job.setJarByClass(GrayImageHistogram.class);
		...
		public static void main(String[] args) throws Exception {
			int res = ToolRunner.run(new GrayImageHistogram(), args);
			System.exit(res);
		}
	}
